...
|`config.taskRouter`||
|`config.taskRouter.baseUrl`|Base URL for API provider (e.g., "https://dashscope.aliyuncs.com/compatible-mode/v1")|
|`config.taskRouter.plannerModel`||
|`config.taskRouter.apiKey`||
|`config.taskRouter.complexityThreshold`|Complexity score threshold above which a task is decomposed. Default 0.6.|
|`config.taskRouter.maxSubtasks`|Maximum number of DAG nodes when decomposing. Default 8.|
|`config.taskRouter.routingStrategy`|Routing strategy: "llm" (default) or "vector".|
|`config.taskRouter.embeddingModel`|Embedding model name. Required when strategy = vector.|
|`config.knowledge`|Evolutionary Memory / Knowledge Management configuration.|
|`config.knowledge.workingMemoryCapacity`|Working-memory ring-buffer capacity (default: 1000).|
|`config.knowledge.kdnEndpoint`|KDN endpoint, e.g. "http://kdn:9000". If unset, KDN integration is disabled.|
|`config.knowledge.sessionTtlSecs`|Session idle TTL in seconds before eviction (default: 1800 = 30 min).|
|`binds`||
|`binds[].port`||
|`binds[].listeners`||
