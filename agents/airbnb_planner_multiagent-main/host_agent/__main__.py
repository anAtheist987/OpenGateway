...
class HostAgentExecutor:
    ...
    async def execute(self, context, queue):
        ...
        # Run the orchestrator once: forward events to agent-listener (port 8084)
        # and collect the final text for A2A response delivery.
        last_final_text = await process_message_and_forward(user_text, [])
...
async def process_message_and_forward(message: str, history: list[dict]) -> str:
    """Run the agent ONCE, forward events to port 8084, and return the final text.

    - function_call events  -> POST /chat/call
    - function_response events -> POST /chat/function
    - final responses       -> POST /chat/final
    ...
    async with aiohttp.ClientSession() as http_session:
        try:
            ...
            print(f'GenAI ClientError: {e}')
            await _post_json(http_session, '/chat/final', {'error': str(e)})
        except Exception as e:
