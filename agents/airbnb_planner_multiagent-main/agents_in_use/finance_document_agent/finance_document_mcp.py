...
"""Finance-only MCP server for document extraction."""
import base64
...
mcp = FastMCP('finance_document_extract')
...
def _finance_notices(data: dict[str, Any]) -> list[dict[str, Any]]:
    notices = data.get('notices', [])
    return [
        n for n in notices if str(n.get('department_id', '')).lower() == FINANCE_DEPARTMENT_ID
    ]
...
async def _extract_with_llm(doc_text: str, categories: list[str] | None) -> dict[str, Any]:
    ...
    prompt = f"""Extract structured information from the finance reimbursement policy document below.
{scope}

Focus on: expense scope, grade-based standards, invoice requirements, approval nodes, audit checkpoints.
...
@mcp.tool()
async def list_finance_notices(limit: int = 20) -> str:
    """List finance department notices only."""
    try:
        data = _load_portal_data()
    except Exception as e:
        return json.dumps({'notices': [], 'warnings': [str(e)]}, ensure_ascii=False, indent=2)
    notices = _finance_notices(data)
    brief = [
        {
            'id': n.get('id'),
            'title': n.get('title'),
...
@mcp.tool()
async def search_finance_notices(query: str, limit: int = 5) -> str:
    """Search finance department notices by keyword."""
    try:
        data = _load_portal_data()
    except Exception as e:
        return json.dumps({'notices': [], 'warnings': [str(e)]}, ensure_ascii=False, indent=2)
...
@mcp.tool()
async def extract_finance_notices(
    notice_ids: list[str],
    categories: list[str] | None = None,
    include_attachment: bool = True,
) -> str:
    """Extract policy details from selected finance notice IDs."""
    if not notice_ids:
    ...
    notice_map = {
        str(n.get('id')): n
        for n in _finance_notices(data)
    }
    missing = []
    selected_texts = []
    ...
    if not selected_texts:
        return _empty_with_warning(f'No valid finance notices found: {", ".join(missing)}')
    merged_text = '\n\n-----\n\n'.join(selected_texts)
...
async def extract_constraints_and_process(
    ...
    """Extract structured finance policy information from plain text."""
...
async def extract_from_file(
    ...
    """Extract structured finance policy information from file."""
    try:
...
async def extract_from_html(
    ...
    """Extract structured finance policy information from HTML."""
