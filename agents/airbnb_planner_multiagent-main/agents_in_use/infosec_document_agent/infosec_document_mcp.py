...
"""InfoSec-only MCP server for document extraction."""
import base64
...
def _infosec_notices(data: dict[str, Any]) -> list[dict[str, Any]]:
    notices = data.get('notices', [])
    return [
        n for n in notices if str(n.get('department_id', '')).lower() == INFOSEC_DEPARTMENT_ID
    ]
...
async def _extract_with_llm(doc_text: str, categories: list[str] | None) -> dict[str, Any]:
    ...
    prompt = f"""Extract structured information from the information security policy document below.
{scope}

Focus on: device boundary, data boundary, cross-border transfer controls, approval chain, audit trail.
...
@mcp.tool()
async def list_infosec_notices(limit: int = 20) -> str:
    """List infosec department notices only."""
    try:
        data = _load_portal_data()
    except Exception as e:
        return json.dumps({'notices': [], 'warnings': [str(e)]}, ensure_ascii=False, indent=2)
    notices = _infosec_notices(data)
    brief = [
        {
            'id': n.get('id'),
            'title': n.get('title'),
...
@mcp.tool()
async def search_infosec_notices(query: str, limit: int = 5) -> str:
    """Search infosec department notices by keyword."""
    try:
        data = _load_portal_data()
    except Exception as e:
        return json.dumps({'notices': [], 'warnings': [str(e)]}, ensure_ascii=False, indent=2)
    ...
    for n in notices:
        ...
        if keywords:
            ...
            if matched_keywords == len(keywords):
                score += 2
        if '涉密' in text or '跨境' in text or '敏感' in text:
            score += 1
...
@mcp.tool()
async def extract_infosec_notices(
    notice_ids: list[str],
    categories: list[str] | None = None,
    include_attachment: bool = True,
) -> str:
    """Extract policy details from selected infosec notice IDs."""
    if not notice_ids:
    ...
    notice_map = {
        str(n.get('id')): n
        for n in _infosec_notices(data)
    }
    missing = []
    selected_texts = []
    ...
    if not selected_texts:
        return _empty_with_warning(f'No valid infosec notices found: {", ".join(missing)}')
    merged_text = '\n\n-----\n\n'.join(selected_texts)
...
async def extract_constraints_and_process(
    ...
    """Extract structured infosec policy information from plain text."""
...
async def extract_from_file(
    ...
    """Extract structured infosec policy information from file."""
    try:
...
async def extract_from_html(
    ...
    """Extract structured infosec policy information from HTML."""
