...
"""InfoSec document extraction agent executor (A2A, deterministic flow)."""
import json
import logging
...
from infosec_document_mcp import (
    extract_infosec_notices,
    list_infosec_notices,
    search_infosec_notices,
)
...
class InfoSecDocumentExecutor(AgentExecutor):
    """Deterministic infosec executor with fixed MCP call path."""
    ...
    async def _build_response(self, user_text: str) -> str:
        try:
            search_data = self._json_loads(
                await search_infosec_notices(query=user_text, limit=3)
            )
            notice_ids = self._pick_notice_ids(search_data)
            if not notice_ids:
                list_data = self._json_loads(await list_infosec_notices(limit=3))
                ...
                return json.dumps(
                    {
                        'process_steps': [],
                        'material_checklist': [],
                        'warnings': ['未找到保密/信息安全部门可抽取公告'],
                    },
                    ensure_ascii=False,
                )

            extracted = self._json_loads(
                await extract_infosec_notices(
                    notice_ids=notice_ids,
                    categories=None,
                    include_attachment=True,
                )
            )
            route_note = f'infosec_document_agent notices={",".join(notice_ids)}'
            ...
            return json.dumps(
                {
                    'process_steps': [],
                    'material_checklist': [],
                    'warnings': [f'infosec agent execution failed: {e!s}'],
                },
                ensure_ascii=False,
            )
