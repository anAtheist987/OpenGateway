...
"""InfoSec-specific document extraction agent."""
import os
import sys
...
def create_infosec_document_agent() -> LlmAgent:
    ...
    mcp_server_script = os.path.join(base_dir, 'infosec_document_mcp.py')

    instruction = """You are the InfoSec Department Document Agent in an A2A system.

You only process secrecy / information-security policy extraction and checklist generation.
Your source data is reimbursement portal notices limited to infosec department.
...
Default workflow:
1) Search notices with infosec keywords from user query.
2) Extract from top relevant notices (<= 3 by default).
3) Focus on security compliance: device boundary, data handling, cross-border controls, approval and audit traces.
...
Respond in the same language as the user."""
    ...
    return LlmAgent(
        ...
        name='infosec_document_agent',
        description='Extracts infosec department policy process and material checklist from portal notices.',
