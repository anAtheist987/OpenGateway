...
Ensure that all responses include the detailed output from the tools used and are formatted clearly."""
    ...
    return LlmAgent(
        model=LiteLlm(model=LITELLM_MODEL, api_key=ANTHROPIC_API_KEY, api_base=ANTHROPIC_API_BASE),
        name='weather_agent',
            ...
            MCPToolset(
                connection_params=StdioConnectionParams(
                    server_params=StdioServerParameters(
                        ...
                        args=[os.path.join(os.path.dirname(os.path.abspath(__file__)), 'weather_mcp.py')],
                    ),
                    timeout=60.0,
                ),
            )
