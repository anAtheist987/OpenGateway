...
_PROXY_URL = os.getenv("https_proxy") or os.getenv("http_proxy") or "http://192.168.1.204:7890"

# --- Configuration & Constants ---
BASE_URL = 'https://api.weather.gov'
USER_AGENT = 'weather-agent'
REQUEST_TIMEOUT = 30.0
GEOCODE_TIMEOUT = 20.0  # Timeout for geocoding requests

# --- Shared HTTP Client (with proxy) ---
...
# --- Geocoding Setup ---
# Inject proxy via environment variables for geopy
...
async def get_forecast_by_city(city: str, state: str) -> str:
    ...
    try:
        # Run synchronous geocode in thread to avoid blocking the event loop
        loop = asyncio.get_event_loop()
        location = await loop.run_in_executor(
            None, lambda: geolocator.geocode(query, timeout=GEOCODE_TIMEOUT)
        )
        if location is None:
        ...
        import traceback
        traceback.print_exc()
        return f"Geocoding exception: {e}"
