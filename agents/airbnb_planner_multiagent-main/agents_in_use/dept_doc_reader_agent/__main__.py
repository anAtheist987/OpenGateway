...
from dept_doc_reader_agent import create_dept_doc_reader_agent
from dept_doc_reader_executor import DeptDocReaderExecutor
...
def main(host: str = DEFAULT_HOST, port: int = DEFAULT_PORT):
    ...
    skill = AgentSkill(
        id='dept_doc_reader_extraction',
        name='Extract shared department policy clauses',
        description='Extract process and checklist from procurement/foreign/safety notices',
        tags=[
            'document',
            'procurement',
            'foreign',
        ...
        examples=[
            '提取采购/集采部门公告中的流程与材料',
            'Extract process checklist for foreign affairs department notice',
        ],
    )
    ...
    agent_card = AgentCard(
        ...
        description='Shared document reader for procurement/foreign/safety department notices.',
        url=app_url,
        version='1.0.0',
    ...
    adk_agent = create_dept_doc_reader_agent()
    runner = Runner(
    ...
    agent_executor = DeptDocReaderExecutor(runner, agent_card)
