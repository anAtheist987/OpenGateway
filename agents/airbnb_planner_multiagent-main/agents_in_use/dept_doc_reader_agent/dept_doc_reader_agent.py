...
"""Shared document reader agent for procurement/foreign/safety departments."""
import os
import sys
...
def create_dept_doc_reader_agent() -> LlmAgent:
    ...
    mcp_server_script = os.path.join(base_dir, 'dept_doc_reader_mcp.py')

    instruction = """You are a shared Department Document Reader agent in an A2A system.

You serve ONLY these departments:
- procurement
- foreign
- safety
...
Rules:
1) If department_id is missing in a department-specific request, return warning asking host to pass one.
2) For notice extraction, always pass department_id.
3) Never handle finance or infosec requests; return warning asking host to call dedicated agents.
...
Respond in the same language as user."""
    ...
    return LlmAgent(
        ...
        name='dept_doc_reader_agent',
        description='Shared document reader for procurement/foreign/safety policy extraction and checklist generation.',
