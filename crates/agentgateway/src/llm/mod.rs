...
impl AIProvider {
	...
	async fn process_request(
		...
		if let Some(log) = log {
			...
			// Extract prompt prefix for KDN fingerprinting (first 512 bytes of concatenated messages).
			if original_format.supports_prompt_guard() && log.prompt_prefix.is_none() {
				...
				let truncated = std::str::from_utf8(&bytes[..bytes.len().min(512)])
					.unwrap_or("")
					.to_string();
				if !truncated.is_empty() {
					log.prompt_prefix = Some(truncated);
				}
			}
		}
