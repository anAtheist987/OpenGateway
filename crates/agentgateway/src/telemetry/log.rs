...
pub struct RequestLog {
	...
	pub knowledge: Option<Arc<crate::knowledge::KnowledgeHandle>>,
	/// Session identifier from the `X-Session-ID` request header.
	pub session_id: Option<String>,
	/// First 512 bytes of the LLM prompt, used for KDN fingerprinting.
	pub prompt_prefix: Option<String>,
	/// Full LLM response text, captured for working-memory replay.
...
impl Drop for DropOnLog {
	fn drop(&mut self) {
		...
		// Evolutionary Memory capture — fire-and-forget, non-blocking.
		if let Some(kh) = log.knowledge.take() {
			let route_key = route_identifier
				.route
				.as_deref()
				.map(agent_core::strng::new)
				.unwrap_or_default();
			let backend = log
				.backend_info
				.as_ref()
			...
			let llm_model = log
				.llm_request
				.as_ref()
				.map(|r| agent_core::strng::new(r.request_model.as_str()));
			let outcome = match log.status.as_ref().map(|s| s.as_u16()) {
				Some(s) if s >= 500 => crate::knowledge::working_memory::Outcome::Failure { status: s },
				_ => crate::knowledge::working_memory::Outcome::Success,
			};
			// Extract LLM completion text for working-memory replay.
			// Falls back to any previously captured value (e.g. set by policy layer).
			let response_content = log.response_content.take().or_else(|| {
				llm_response
					.as_ref()
					.and_then(|l| l.response.completion.as_ref())
					.map(|cs| cs.join(""))
					.filter(|s| !s.is_empty())
			});
			kh.capture(crate::knowledge::CaptureArgs {
				session_id: log.session_id.take(),
				route_key,
				backend,
				llm_model,
				prompt_prefix: log.prompt_prefix.take(),
				response_content,
				agent_id: None,
				outcome,
				latency: duration,
