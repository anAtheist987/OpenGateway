...
async fn apply_backend_policies(
	...
	if let Some(a2a) = a2a {
		let (a2a_type, task_text) = a2a::apply_to_request(a2a, req).await;
		if let Some(text) = task_text {
			log.add(|l| {
				if l.prompt_prefix.is_none() {
					l.prompt_prefix = Some(text);
				}
			});
		}
...
impl HTTPProxy {
	pub async fn proxy(
		...
		log.knowledge = Some(Arc::clone(&self.inputs.knowledge));
		log.session_id = req
			.headers()
			.get("x-session-id")
...
async fn make_backend_call(
	...
	// For plain HTTP proxy routes carrying A2A messages (no explicit a2a policy),
	// attempt opportunistic extraction of the task text for working-memory fingerprinting.
	// Handles both Python SDK ("parts") and Rust SDK ("content") field names.
	if backend_call.backend_policies.a2a.is_none()
		&& backend_call.backend_policies.llm_provider.is_none()
		&& req.method() == http::Method::POST
		&& log
			.as_deref()
			.is_some_and(|l| l.knowledge.is_some() && l.prompt_prefix.is_none())
		&& let crate::http::WellKnownContentTypes::Json =
			crate::http::classify_content_type(req.headers())
		&& let Ok(v) = crate::json::inspect_body::<serde_json::Value>(&mut req).await
		&& let Some(text) = a2a::try_extract_text_from_value(&v)
	{
		log.add(|l| {
			l.prompt_prefix = Some(text);
		});
