...
use crate::{Config, ProxyInputs, client, knowledge, mcp, proxy, state_manager};
...
pub async fn run(config: Arc<Config>) -> anyhow::Result<Bound> {
	...
	let knowledge_handle = {
		let mut kh = knowledge::KnowledgeHandle::new(
			config.knowledge.working_memory_capacity,
			config.knowledge.session_ttl_secs,
		);
		if let Some(ref endpoint) = config.knowledge.kdn_endpoint {
			kh = kh.with_kdn(knowledge::kdn_client::KdnClient::new(endpoint.clone()));
		}
		Arc::new(kh)
	};
	admin_server.set_knowledge(Arc::clone(&knowledge_handle));
