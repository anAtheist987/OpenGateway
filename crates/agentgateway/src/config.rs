...
pub fn parse_config(contents: String, filename: Option<PathBuf>) -> anyhow::Result<Config> {
	...
	Ok(crate::Config {
		...
		threading_mode,
		backend: raw.backend,
		task_router: raw.task_router,
		knowledge: crate::KnowledgeConfig {
			working_memory_capacity: raw.knowledge.working_memory_capacity.unwrap_or(1000),
			kdn_endpoint: raw.knowledge.kdn_endpoint,
			session_ttl_secs: raw
				.knowledge
				.session_ttl_secs
				.unwrap_or(crate::knowledge::DEFAULT_SESSION_TTL_SECS),
		},
