...
//! Proxy matchers
//!
//! This module contains different matchers to configure rules for when a agentgateway
...
//!
//! An [`Intercept`] includes the destination for the agentgateway, and any parsed
...
/// A agentgateway matcher, usually built from environment variables.
pub struct Matcher {
...
/// A matched agentgateway,
///
/// This is returned by a matcher if a agentgateway should be used.
#[derive(Clone)]
...
/// A filter for agentgateway matchers.
///
...
impl Matcher {
	...
	///
	/// This constructor is always available, but if the `client-agentgateway-system`
	...
	/// Check if the destination should be intercepted by a agentgateway.
	///
	/// If the agentgateway rules match the destination, a new `Uri` will be returned
	/// to connect to.
...
impl Intercept {
	/// Get the `http::Uri` for the target agentgateway.
	...
	/// that should be sent with whatever protocol the agentgateway handshake uses.
	///
	/// # Example
	///
	/// ```rust
	/// # use hyper_util::client::agentgateway::matcher::Matcher;
	/// # let uri = http::Uri::from_static("https://hyper.rs");
	/// let m = Matcher::builder()
	///     .all("socks5h://Aladdin:opensesame@localhost:8887")
	///     .build();
	///
	/// let agentgateway = m.intercept(&uri).expect("example");
	/// let auth = agentgateway.raw_auth().expect("example");
...
impl Builder {
	...
	/// Set the target agentgateway for all destinations.
	...
	/// Set the target agentgateway for HTTP destinations.
	...
	/// Set the target agentgateway for HTTPS destinations.
	...
	/// Set the "no" agentgateway filter.
	///
	...
	/// (and therefore would bypass the agentgateway):
	/// * `http://google.com/`
	/// * `http://www.google.com/`
	/// * `http://192.168.1.42/`
	///
	...
	builder = builder.scheme(match uri.scheme() {
		Some(s) => {
			if s == &http::uri::Scheme::HTTP || s == &http::uri::Scheme::HTTPS {
				...
				// can't use this agentgateway scheme
				return None;
			}
		},
...
impl NoProxy {
	...
	/// Returns a new no-agentgateway configuration based on a `no_proxy` string (or `None` if no variables
	/// are set)
	/// The rules are as follows:
