...
import CallTree from "@/components/playground/CallTree";
...
export default function PlaygroundPage() {
  ...
  // Connect to local agent listener for real-time call tree updates
  useEffect(() => {
    let es: EventSource | null = null;

    async function fetchTree() {
      try {
        const res = await fetch("http://localhost:8084/tree");
        if (res.ok) {
    ...
    try {
      es = new EventSource("http://localhost:8084/events");
      es.onmessage = (ev) => {
        try {
    ...
    try {
      if (backendType === "mcp") {
        ...
        try {
          ...
          setA2aState((prev) => ({ ...prev, agentCard, skills, selectedSkill: skills[0] ?? null }));
          toast.success(
